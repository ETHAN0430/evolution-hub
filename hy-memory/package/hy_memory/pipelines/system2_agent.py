...
7. **输出前自查：检查每条 add_edge 操作是否都有 `edge_type` 字段。如果有遗漏，立即补上最合适的边类型。**"""
    else:
        output_instructions = """
...
6. **Self-verify before output: Check every add_edge operation for `edge_type`. If any is missing, add the most appropriate edge type.**"""
