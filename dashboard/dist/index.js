(function () {
  ...
  var NODES = {
    ...
    'Agent Init': {file: 'agent/agent_init.py', loc: 'init_agent', x: 510, y: 150, group: 'pipeline', desc: '初始化 AIAgent 的地方。只在新建会话时跑一次：\n1. 设置 platform（cli/tui/api_server/...）\n2. 构建并缓存 system prompt\n3. 组装可用工具列表（含 MCP）\n4. 初始化会话状态（session_id、source、model 等）'},
    '输入清洗': {file: 'agent/turn_context.py', loc: 'build_turn_context', x: 510, y: 220, group: 'pipeline', desc: '每轮 Turn 的入口。清洗用户输入（如去掉非法 surrogate 字符），并把用户消息追加到对话历史中。'},
    'MCP 刷新': {file: 'tools/mcp_tool.py', loc: 'refresh_agent_mcp_tools', x: 510, y: 290, group: 'pipeline', desc: '每轮开头刷新 MCP 工具列表：检查是否有新连上的 MCP server，把新工具加入当前可用工具快照。'},
    '记忆预取': {file: 'agent/memory_manager.py', loc: 'prefetch_all', x: 510, y: 360, group: 'pipeline', desc: '用当前用户消息向 MemoryManager 发起 prefetch，把相关记忆（MEMORY.md、USER.md、HY Memory 等）提前查出来，供后续 prompt 使用。'},
    '插件上下文': {file: 'hermes_cli/plugins.py', loc: 'invoke_hook', x: 510, y: 430, group: 'pipeline', desc: '调用 pre_llm_call 插件钩子，把插件返回的额外上下文注入到用户消息中。'},
    '消息构建': {file: 'agent/system_prompt.py', loc: 'build_system_prompt', x: 510, y: 500, group: 'pipeline', desc: '把你的问题、之前的对话、以及查到的记忆，打包成一封发给 AI 的“信”。'},
    'LLM API': {file: 'agent/conversation_loop.py', loc: 'run_conversation', x: 510, y: 640, group: 'pipeline', desc: '真正去调用 AI 模型的地方。把准备好的“信”发出去，等 AI 回信。'},
    '工具执行': {file: 'agent/tool_executor.py', loc: 'execute_tool_calls_concurrent', x: 660, y: 640, group: 'pipeline', desc: '让 AI 可以动手做事，比如查资料、读写文件、搜索网页等。'},
    '上下文压缩': {file: 'agent/context_compressor.py', loc: 'ContextCompressor', x: 510, y: 570, group: 'pipeline', desc: '进入 LLM 前或工具结果返回后，如果上下文超过阈值，先压缩再交给 LLM。'},
    ...
    'Turn End': {file: 'agent/turn_finalizer.py', loc: 'finalize_turn', x: 810, y: 380, group: 'pipeline', desc: '最终收尾：统计 token/cost、提取 reasoning、组装 result 返回给调用方。'},
